...
def find_features(equidistant, index, csv1, csv2, csv3):
	...
	if(len(equidistant) != 0):
		for lis in equidistant[0]:
			...
			dis_hyp = float(sqrt((lis[2][0]-lis[0][0])**2 + (lis[2][1]-lis[0][1])**2))
			dis_adj = float(sqrt((lis[1][0]-lis[0][0])**2 + (lis[1][1]-lis[0][1])**2))
			dis_opp = float(sqrt((lis[2][0]-lis[1][0])**2 + (lis[2][1]-lis[1][1])**2))
			if dis_hyp== 0:
				lap+=1
			...
			if(dis_hyp !=0):
				...
				m1 = float(dis_opp/dis_hyp)
				#m2 = (lis[2][1]-lis[1][1])/float(dis)
				m2 = float(dis_adj/dis_hyp)
				cos_sin.append([m1,m2])
			csv1.append(m1)
			csv2.append(m2)
			csv3.append(index)
	...
	#print len(sine)
	#print(cos_sin)
	#csv.append([m1, m2, index])		#index will give class number
